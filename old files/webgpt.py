import os
import zipfile
import subprocess
from flask import Flask, request, send_file, render_template
from werkzeug.utils import secure_filename
import shutil
import logging

app = Flask(__name__)

# Define folder paths
UPLOAD_FOLDER = 'uploads'
IMAGES_FOLDER = 'images'
OUTPUT_FOLDER = 'output'
FINALOUTPUT_FOLDER = 'finaloutput'
INPUT_FOLDER = 'input'

# Allowed file types
ALLOWED_EXTENSIONS = {'tiff', 'tif'}

app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER

# Set up logging for debugging
logging.basicConfig(level=logging.DEBUG)

# Check if file is a valid type
def allowed_file(filename):
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

def clear_folder(folder_path):
    """Helper function to clear all files in a folder, 
    but keep the directory structure intact."""
    for root, dirs, files in os.walk(folder_path):
        # Only remove files, leave the directories intact
        for file in files:
            file_path = os.path.join(root, file)
            if os.path.isfile(file_path):
                os.remove(file_path)

# Helper function to run subprocesses in order with the correct working directory
def run_subprocess(script, args, cwd=None):
    try:
        # If a working directory (cwd) is provided, run the subprocess in that directory
        logging.debug(f"Running subprocess: {script} with args {args} in directory {cwd}")
        subprocess.run([script] + args, check=True, timeout=600, cwd=cwd)  # Set an appropriate timeout
    except subprocess.TimeoutExpired:
        logging.error(f"Timeout expired for {script} with args {args}")
    except subprocess.CalledProcessError as e:
        logging.error(f"Error occurred while running {script}: {e}")

# Handle the upload of the image
@app.route('/')
def index():
    return render_template('index.html')

@app.route('/upload', methods=['POST'])
def upload_file():
    if 'file' not in request.files:
        return 'No file part', 400
    
    file = request.files['file']
    
    if file.filename == '':
        return 'No selected file', 400

    if file and allowed_file(file.filename):
        filename = secure_filename(file.filename)
        filepath = os.path.join(UPLOAD_FOLDER, filename)
        file.save(filepath)

        # Set working directory for subprocesses
        cwd = os.getcwd()

        # Sequentially run subprocesses with proper exception handling
        try:
            run_subprocess('python3', ['normalize2.py'], cwd=cwd)
            run_subprocess('python3', ['splitimagev2.py', filepath], cwd=cwd)
            run_subprocess('python3', ['appcsv3.py'], cwd=cwd)
            run_subprocess('python3', ['mergeimage.py'], cwd=cwd)
            run_subprocess('python3', ['readingcsv2.py'], cwd=cwd)

            # Create a ZIP file from the finaloutput folder
            zip_filename = 'finaloutput.zip'
            zip_filepath = os.path.join(FINALOUTPUT_FOLDER, zip_filename)

            with zipfile.ZipFile(zip_filepath, 'w') as zipf:
                for root, dirs, files in os.walk(FINALOUTPUT_FOLDER):
                    for file in files:
                        if file == zip_filename:
                            continue
                        zipf.write(os.path.join(root, file), os.path.relpath(os.path.join(root, file), FINALOUTPUT_FOLDER))

            # Provide download link for final output ZIP file
            return render_template('index.html', zip_file=zip_filename)

        except Exception as e:
            logging.error(f"Error during image processing: {e}")
            return f"An error occurred: {e}", 500

    return 'Invalid file format', 400

@app.route('/download/<filename>', methods=['GET'])
def download_file(filename):
    zip_filepath = os.path.join(FINALOUTPUT_FOLDER, filename)
    if os.path.exists(zip_filepath):
        return send_file(zip_filepath, as_attachment=True)

    return 'File not found', 404

@app.route('/clear_files', methods=['POST'])
def clear_files():
    # Clear files after the download request
    clear_folder(UPLOAD_FOLDER)
    clear_folder(IMAGES_FOLDER)
    clear_folder(OUTPUT_FOLDER)
    clear_folder(INPUT_FOLDER)
    clear_folder(FINALOUTPUT_FOLDER)
    return 'Files cleared', 200

if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5000, threaded=True)  # Use threaded mode to handle multiple requests
